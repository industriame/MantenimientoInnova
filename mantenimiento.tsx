...
import {
  ...
  ChevronLeft, AlertTriangle, CheckCircle2, Clock, DollarSign, Building2, Layers,
...
function seedData() {
  ...
  const planes = planDefs.map((p, i) => {
    ...
    return {
      id: uid("plan"), tarea: p.tarea, procedimiento: p.proc, categoria: p.cat,
  ...
  const mkOT = (n, plan, ap, estado, fecha, tecnicoId, mats, matEstado) => ({
    ...
    planId: plan.id, tarea: plan.tarea, procedimiento: plan.procedimiento,
  ...
  const servicios = [
    {
      ...
      proveedor: "Clima Andino S.A.", presupuesto: 180,
    ...
    },
    {
      ...
      proveedor: "Fire Tech Ecuador", presupuesto: 240,
...
const serviciosDeSedeMes = (data, sedeId, mes) =>
  (data.servicios || []).filter((s) => s.sedeId === sedeId && mesKey(s.fecha) === mes);
...
function presupuestoSedeMes(data, sedeId, mes) {
  ...
  const costoServicios = servicios.reduce((s, x) => s + (Number(x.presupuesto) || 0), 0);
...
function indicadoresMes(data, sedeIds, mes) {
  ...
  sedeIds.forEach((id) => {
    ...
    costoServicios += serviciosDeSedeMes(data, id, mes).reduce((a, x) => a + (Number(x.presupuesto) || 0), 0);
...
function avancePlan(data, sedeIds) {
  ...
  (data.planes || []).forEach((plan) => {
    (plan.aplicaciones || []).forEach((ap) => {
      ...
      if (rel.some((o) => o.estado === "programada" || o.estado === "en_proceso")) { cats.ejecucion++; return; }
...
function getPendientes(data) {
  ...
  (data.planes || []).forEach((plan) => {
    (plan.aplicaciones || []).forEach((ap) => {
      ...
      if (rel.some((o) => o.estado === "programada" || o.estado === "en_proceso")) return;
      const ultima = rel
...
function normalizeData(raw) {
  ...
  d.planes = arr(d.planes).map((p) => ({ ...p, aplicaciones: arr(p.aplicaciones) }));
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setData(seed);
        try { await window.storage.set(STORAGE_KEY, snapshotRef.current, true); } catch (_) {}
      } finally {
        setLoading(false);
      }
    })();
...
function useAcciones(data, persist) {
  return useMemo(() => ({
    updateOrden: (id, patch) =>
      persist({ ...data, ordenes: data.ordenes.map((o) => (o.id === id ? { ...o, ...patch } : o)) }),
    updateSolicitud: (id, patch) =>
      persist({ ...data, solicitudes: data.solicitudes.map((s) => (s.id === id ? { ...s, ...patch } : s)) }),
    ...
    updateActividad: (item, patch) =>
      item.tipo === "preventivo"
        ? persist({ ...data, ordenes: data.ordenes.map((o) => (o.id === item.id ? { ...o, ...patch } : o)) })
        : persist({ ...data, solicitudes: data.solicitudes.map((s) => (s.id === item.id ? { ...s, ...patch } : s)) }),
  }), [data, persist]);
}
  ...
  return (
    <div>
      ...
      <div className="space-y-1.5">
        {materiales.map((m) => (
          <div key={m.id} className="border rounded-md p-2" style={bLine}>
            {puedeListar ? (
              <div className="space-y-1.5">
                <input value={m.nombre} onChange={(e) => set(m.id, { nombre: e.target.value })} placeholder="Material"
                  className="w-full border rounded px-2 py-1 text-xs outline-none" style={inputStyle} />
                <div className="flex gap-1.5 items-center">
                  <input type="number" min="0" value={m.cantidad} onChange={(e) => set(m.id, { cantidad: e.target.value })} placeholder="Cant."
                    className="w-16 border rounded px-2 py-1 text-xs outline-none" style={inputStyle} />
                  <input value={m.unidad} onChange={(e) => set(m.id, { unidad: e.target.value })} placeholder="Unidad"
                    className="flex-1 border rounded px-2 py-1 text-xs outline-none" style={inputStyle} />
              ...
              <div className="flex items-center justify-between text-xs gap-2">
                ...
                {puedeCostear ? (
                  <div className="flex items-center gap-1 shrink-0">
                    <input value={m.unidad} onChange={(e) => set(m.id, { unidad: e.target.value })} placeholder="Unid."
                    ...
                    <input type="number" min="0" step="0.01" value={m.costoUnitario} onChange={(e) => set(m.id, { costoUnitario: e.target.value })}
      ...
      {puedeListar && (
        <button
          onClick={() => onUpdate({
            materiales: [...materiales, { id: uid("mat"), nombre: "", cantidad: 1, unidad: "u", costoUnitario: 0 }],
      ...
      {puedeListar && materiales.length > 0 && (
        puedeEnviar ? (
          ...
          <p className="text-[10px] mt-2" style={cSlate}>Marca la actividad "En proceso" para enviar los materiales a costeo.</p>
        )
      )}
...
function ConsumoStock({ item, stockSede, onRegistrar, onQuitar, readOnly }) {
  ...
  return (
    <div>
      ...
      {!readOnly && (
        stockSede.length === 0 ? (
          <p className="text-[10px]" style={cSlate}>Esta sede aún no tiene stock cargado.</p>
        ) : (
          <>
              ...
              <select value={sel} onChange={(e) => { setSel(e.target.value); setCant(1); }}
                ...
                {stockSede.map((x) => (
                  <option key={x.id} value={x.id} disabled={x.cantidad <= 0}>
                    {x.nombre} · {x.cantidad} {x.unidad} {x.cantidad <= 0 ? "(agotado)" : ""}
                  </option>
                ))}
              </select>
              <input type="number" min="1" max={art?.cantidad || 999} value={cant}
            ...
            {art && (
              <p className="text-[10px] mt-1" style={{ color: excede ? COLORS.rojo : COLORS.slate }}>
                {excede
...
function VistaSolicitante({ data, persist, user, onLogout, ultimaSync }) {
  ...
  const calificar = (id, patch) =>
    persist({ ...data, solicitudes: data.solicitudes.map((x) => (x.id === id ? { ...x, ...patch } : x)) });
  ...
  const crearSolicitud = (ubic, form) => {
    ...
    persist({ ...data, solicitudes: [nueva, ...data.solicitudes], solCounter: n + 1 });
  ...
  return (
    <div className="border rounded-md p-2.5" style={{ borderColor: COLORS.line, borderLeft: `3px solid ${tipoColor}`, background: "white" }}>
      ...
      <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
        ...
        {esServ && Number(item.presupuesto) > 0 && <Chip color={COLORS.orange}>{money(item.presupuesto)}</Chip>}
...
function ArbolPendientes({ sedes, todosLosSedes, usuarios, pendientes, onActivar }) {
                ...
                {["preventivo", "correctivo", "servicio"].map((tipo) => {
                  ...
                  return (
                    <GrupoTipo key={`${sede.id}_${tipo}`} tipo={tipo} items={grupo}
                      todosLosSedes={todosLosSedes} usuarios={usuarios} onActivar={onActivar} />
                  );
                })}
              </div>
            )}
      ...
      {open && (
        <div className="px-3 pb-3 space-y-3 border-t pt-3" style={bLine}>
          ...
          {esPrev && item.procedimiento && (
            <Field label="Procedimiento">
              <p className="text-xs whitespace-pre-wrap rounded-md p-2.5" style={{ background: COLORS.cream, color: COLORS.charcoal }}>{item.procedimiento}</p>
            </Field>
          )}
          ...
          {!esPrev && (
            <Field label="Resolución">
              <textarea value={resolucion} onChange={(e) => setResolucion(e.target.value)} rows={2}
                placeholder="Qué se hizo para resolverlo" className={`${inputCls} resize-none`} style={inputStyle} />
            </Field>
          )}
          ...
          <MaterialesPanel item={item} rol={rol} onUpdate={(patch) => acciones.updateActividad(item, patch)} puedeEnviar={estado === "en_proceso"} />
...
function FormHallazgoTecnico({ sedes, user, onSubmit, onClose }) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div>
          <p className="font-semibold" style={cSlate}>Reporta</p>
          <p style={cChar}>{user.nombre}</p>
        </div>
        <div>
          <p className="font-semibold" style={cSlate}>Fecha y hora</p>
      ...
      <Field label="Hallazgo">
        <textarea value={descripcion} onChange={(e) => setDescripcion(e.target.value)} rows={3}
          placeholder="Describe lo encontrado en la inspección..." className={`${inputCls} resize-none`} style={inputStyle} />
      </Field>
      ...
      <button disabled={!valido} onClick={() => { onSubmit({ sedeId, faseId, activoId, descripcion, criticidad }); onClose(); }}
        className="w-full py-2.5 rounded-md font-semibold text-sm text-white flex items-center justify-center gap-2 disabled:opacity-40"
        style={{ background: COLORS.orange }}>
    ...
    if (item.tipo === "correctivo") {
      persist({
        ...
        solicitudes: data.solicitudes.map((x) =>
          x.id === item.solicitudId ? { ...x, tecnicoId, fechaProgramada: fecha, estado: "programada" } : x),
      });
    } else {
      ...
      persist({
        ...
        ordenes: [...data.ordenes, {
          ...
          planId: item.planId, tarea: item.tarea, procedimiento: item.procedimiento,
          categoria: item.categoria, frecuencia: item.frecuencia,
          duracionValor: item.duracionValor, duracionUnidad: item.duracionUnidad,
      ...
      {activar && (
        <Modal title="Adelantar actividad" onClose={() => setActivar(null)}>
          <FormActivar item={activar} data={data} soloTecnico={user}
            onConfirm={({ tecnicoId, fecha }) => { activarActividad(activar, tecnicoId, fecha); setActivar(null); }}
...
function AdminSedes({ data, persist }) {
  ...
  const qrUrl = qr ? `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(`https://tudominio.com/reportar?activo=${qr.activoId}`)}` : null;
...
function FormPlan({ data, initial, onSave, onClose, onAddCategoria }) {
  ...
  const submit = () => {
    onSave({
      ...
      tarea: tarea.trim(), procedimiento: procedimiento.trim(), categoria, frecuencia,
  ...
  return (
    <div className="space-y-3">
      ...
      <Field label="Procedimiento" hint="Guía paso a paso para cualquier técnico.">
        <textarea value={procedimiento} onChange={(e) => setProcedimiento(e.target.value)} rows={4}
          placeholder="1. ...&#10;2. ..." className={`${inputCls} resize-none`} style={inputStyle} />
      </Field>
...
function TarjetaPlan({ plan, sedes, onEdit, onDelete }) {
  ...
  return (
    <div className="border rounded-md p-3" style={cardStyle}>
      ...
      <div className="flex items-center gap-3 mt-2 flex-wrap">
        {plan.procedimiento && (
          <button onClick={() => setVerProc(!verProc)} className="flex items-center gap-1 text-[11px] font-semibold" style={cOrange}>
            {verProc ? <ChevronDown size={12} /> : <ChevronRight size={12} />} Procedimiento
          </button>
        )}
        {aps.length > 0 && (
      ...
      {verProc && (
        <p className="text-xs whitespace-pre-wrap mt-2 pt-2.5" style={{ borderTop: `1px solid ${COLORS.line}`, color: COLORS.charcoal }}>{plan.procedimiento}</p>
      )}
      {verUbic && (
...
function FormActivar({ item, data, onConfirm, onClose, soloTecnico }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <button disabled={!tecnicoId} onClick={() => { onConfirm({ tecnicoId, fecha }); onClose(); }}
    ...
    <div onClick={() => ver(act)} title="Ver detalle completo"
      ...
      <div className="flex items-center justify-between gap-2 mt-1">
        ...
        {esServicio && Number(act.presupuesto) > 0 && (
          <span className="text-[10px] font-bold shrink-0" style={cOrange}>{money(act.presupuesto)}</span>
        )}
      </div>
    </div>
  );
...
function Calendario({ data, sedes, tecnicoDefault }) {
  ...
  const shift = (d) => {
    ...
    if (modo === "mes") n.setMonth(n.getMonth() + d); else n.setDate(n.getDate() + d * 7);
    setAncla(n);
  };
  ...
  const titulo = modo === "mes"
    ...
    : (() => { const d = diasSemana; return `${d[0].getDate()} ${MESES[d[0].getMonth()].slice(0, 3)} — ${d[6].getDate()} ${MESES[d[6].getMonth()].slice(0, 3)}`; })();
    ...
    <div>
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <div className="flex gap-1">
          {["mes", "semana"].map((v) => (
            <button key={v} onClick={() => setModo(v)} className="text-xs font-semibold px-3 py-1.5 rounded-md border capitalize"
              style={{ background: modo === v ? COLORS.charcoal : "white", color: modo === v ? "white" : COLORS.slate, borderColor: modo === v ? COLORS.charcoal : COLORS.line }}>{v}</button>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5 mb-3 flex-wrap">
        ...
        <select value={fSede} onChange={(e) => setFSede(e.target.value)} className={selectCls} style={inputStyle}>
          <option value="todas">{sedesVista.length > 1 ? "Todas mis sedes" : "Sede"}</option>
          {sedesVista.map((s) => <option key={s.id} value={s.id}>{s.nombre}</option>)}
        </select>
        ...
        <select value={fSolicitante} onChange={(e) => setFSolicitante(e.target.value)} className={selectCls} style={inputStyle}>
        ...
        <span className="flex items-center gap-1.5 text-[10px]" style={cSlate}>
          ...
          {Object.entries(TIPO_META).map(([k, m]) => (
            <span key={k} className="flex items-center gap-1">
              <span className="w-2.5 h-1 rounded-sm" style={{ background: m.color }} />{m.label}
            </span>
          ))}
        </span>
        <span className="flex items-center gap-1.5 text-[10px]" style={cSlate}>
          ...
          {sedesVista.map((s) => (
            <span key={s.id} className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full" style={{ background: sedeColor(data.sedes, s.id) }} />{s.nombre}
            </span>
          ))}
        </span>
      </div>
            ...
            {semanas.map((d, i) => {
              ...
              return (
                <button key={i} onClick={() => dia.length && setDiaModal(k)}
                  ...
                  style={{ borderColor: hoy ? COLORS.orange : COLORS.line, background: dia.length ? COLORS.cream : "white" }}>
                  <span className="text-[10px] font-semibold" style={{ color: hoy ? COLORS.orange : COLORS.charcoal }}>{d.getDate()}</span>
                  {sedesDia.length > 0 && (
                    <div className="flex gap-0.5 mt-1 flex-wrap justify-center">
                      {sedesDia.slice(0, 3).map((sid) => <span key={sid} className="w-1.5 h-1.5 rounded-full" style={{ background: sedeColor(data.sedes, sid) }} />)}
                    </div>
                  )}
                  {dia.length > 0 && <span className="text-[9px] mt-0.5 font-semibold" style={cSlate}>{dia.length}</span>}
                </button>
              );
            })}
          </div>
      ...
      {modo === "semana" && (
        <div className="space-y-1.5">
          {diasSemana.map((d) => {
            ...
            return (
              <div key={k} className="border rounded-md overflow-hidden" style={{ borderColor: hoy ? COLORS.orange : COLORS.line, background: "white" }}>
                <button onClick={() => dia.length && setAbiertos((p) => ({ ...p, [k]: !p[k] }))}
                  className="w-full flex items-center justify-between px-3 py-2" style={{ background: hoy ? `${COLORS.orange}0D` : "white" }}>
                  <div className="flex items-center gap-2">
                    {dia.length > 0 && (open ? <ChevronDown size={13} color={COLORS.slate} /> : <ChevronRight size={13} color={COLORS.slate} />)}
                    <span className="text-xs font-semibold" style={{ color: hoy ? COLORS.orange : COLORS.charcoal }}>
                      {["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"][(d.getDay() + 6) % 7]} {d.getDate()} {MESES[d.getMonth()].slice(0, 3)}
                    </span>
                  </div>
                  {dia.length > 0 && <Chip color={COLORS.orange}>{dia.length}</Chip>}
                </button>
                {open && <div className="px-3 pb-3 space-y-1.5">{dia.map((a) => <TarjetaAgenda key={a.id} act={a} data={data} />)}</div>}
              </div>
            );
          })}
        </div>
      ...
      {diaModal && (
        <Modal title={`Agenda · ${diaModal}`} onClose={() => setDiaModal(null)}>
          <div className="space-y-2">{(porFecha[diaModal] || []).map((a) => <TarjetaAgenda key={a.id} act={a} data={data} />)}</div>
        </Modal>
      )}
    </div>
  );
...
function PanelProgramacion({ data, sedes, pendientes, onActivar, tecnicoDefault, nota }) {
  return (
    <div className="mt-4 flex flex-col lg:flex-row gap-4">
      <div className="w-full lg:w-1/3 xl:w-1/4">
        ...
        <ArbolPendientes sedes={sedes} todosLosSedes={data.sedes} usuarios={data.usuarios}
          pendientes={pendientes} onActivar={onActivar} />
      </div>
    ...
    if (item.tipo === "correctivo") {
      persist({
        ...
        solicitudes: data.solicitudes.map((s) =>
          s.id === item.solicitudId ? { ...s, tecnicoId, fechaProgramada: fecha, estado: "programada" } : s),
      });
    } else {
      const n = data.otCounter || 1;
      const orden = {
...
function AdminCorrectivos({ data, persist }) {
  ...
  const correctivos = data.solicitudes
    .filter((s) => (fSede === "todas" || s.sedeId === fSede) && (fEstado === "todos" || s.estado === fEstado))
    .sort((a, b) => (a.fecha + a.hora < b.fecha + b.hora ? 1 : -1));
  ...
  return (
    <div className="mt-4 space-y-5">
      <div>
        <SectionTitle count={enCosteo.length}>Materiales pendientes de costeo</SectionTitle>
        <div className="space-y-2">
      ...
      {enAprobacion.length > 0 && (
        <div>
          <SectionTitle count={enAprobacion.length}>Enviados al cliente</SectionTitle>
          <div className="space-y-2">
      ...
      {resueltos.length > 0 && (
        <div>
          ...
          <div className="space-y-2">
            {resueltos.map((i) => <TarjetaCosto key={i.id} item={i} data={data} rol="admin" onUpdate={(p) => acciones.updateActividad(i, p)} />)}
          </div>
        </div>
      )}

      <div>
        <SectionTitle count={correctivos.length}>Todas las solicitudes correctivas</SectionTitle>
        <p className="text-xs mb-2" style={cSlate}>
          Puedes editarlas como técnico (estado, observaciones, foto, materiales) y aprobar sus costos.
        </p>
        <div className="flex gap-2 mb-2 flex-wrap">
          <select value={fSede} onChange={(e) => setFSede(e.target.value)} className="flex-1 min-w-32 border rounded-md px-2 py-2 text-sm bg-white" style={inputStyle}>
            <option value="todas">Todas las sedes</option>
            {data.sedes.map((s) => <option key={s.id} value={s.id}>{s.nombre}</option>)}
          </select>
          <select value={fEstado} onChange={(e) => setFEstado(e.target.value)} className="flex-1 min-w-32 border rounded-md px-2 py-2 text-sm bg-white" style={inputStyle}>
            <option value="todos">Todos los estados</option>
            {Object.keys(ESTADOS).map((e) => <option key={e} value={e}>{ESTADOS[e].label}</option>)}
          </select>
        </div>
        <div className="space-y-2">
          {correctivos.map((sol) => (
          ...
          {correctivos.length === 0 && <Empty>No hay correctivos con este filtro.</Empty>}
        </div>
      </div>
    </div>
  );
...
function FormServicio({ data, initial, onSave, onClose }) {
  ...
  const fase = sede?.fases.find((f) => f.id === faseId);
  const valido = sedeId && faseId && activoId && trabajo.trim() && Number(presupuesto) > 0;
    ...
    <div className="space-y-3">
      ...
      <Field label="Trabajo a realizar">
        <textarea value={trabajo} onChange={(e) => setTrabajo(e.target.value)} rows={3}
          placeholder="Ej. Mantenimiento y recarga de aire acondicionado" className={`${inputCls} resize-none`} style={inputStyle} />
      </Field>

      <Field label="Proveedor / especialista">
        <input value={proveedor} onChange={(e) => setProveedor(e.target.value)} placeholder="Ej. Clima Andino S.A." className={inputCls} style={inputStyle} />
      </Field>
      ...
      <Field label="Estado">
        <select value={estado} onChange={(e) => setEstado(e.target.value)} className="w-full border rounded-md px-2 py-2 text-sm" style={inputStyle}>
          {ESTADOS_SERVICIO.map((e) => <option key={e} value={e}>{ESTADOS[e].label}</option>)}
        </select>
      </Field>

      <button disabled={!valido}
        onClick={() => { onSave({ id: initial?.id, sedeId, faseId, activoId, trabajo: trabajo.trim(), proveedor: proveedor.trim(), presupuesto: Number(presupuesto) || 0, fecha, estado, observaciones: initial?.observaciones || "" }); onClose(); }}
        className="w-full py-2.5 rounded-md font-semibold text-sm text-white disabled:opacity-40" style={{ background: COLORS.orange }}>
...
function AdminServicios({ data, persist }) {
  ...
  const guardar = (srv) => {
    if (srv.id) {
      persist({ ...data, servicios: data.servicios.map((x) => (x.id === srv.id ? { ...x, ...srv } : x)) });
    } else {
      const n = data.srvCounter || 1;
      persist({
        ...data,
        servicios: [...(data.servicios || []), { ...srv, id: uid("srv"), codigo: `SRV-${String(n).padStart(4, "0")}` }],
        srvCounter: n + 1,
      });
    }
  };
  ...
  return (
    <div className="mt-4">
      ...
      <p className="text-xs mb-3" style={cSlate}>
        Trabajos externos de especialidad con presupuesto propio. No consumen el presupuesto mensual de materiales,
        pero sí entran en el costo de mantenimiento por estudiante.
      </p>

      <div className="space-y-2">
        {servicios.map((srv) => (
          <div key={srv.id} className="border rounded-md p-3" style={{ borderColor: COLORS.line, borderLeft: `3px solid #3B6EA5`, background: "white" }}>
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <Chip solid color="#3B6EA5">Servicio</Chip>
              ...
              <div className="flex flex-col items-end gap-1.5 shrink-0">
                <span className="text-sm font-bold" style={cOrange}>{money(srv.presupuesto)}</span>
                <div className="flex items-center gap-1.5">
                  <BotonDetalle item={{ ...srv, tipo: "servicio", tarea: srv.trabajo, fechaProgramada: srv.fecha }} size={13} />
                  <button onClick={() => setModal({ srv })}><Pencil size={13} color={COLORS.slate} /></button>
                  <DeleteBtn onConfirm={() => persist({ ...data, servicios: data.servicios.filter((x) => x.id !== srv.id) })} />
                </div>
              </div>
            </div>
          </div>
        ))}
        {servicios.length === 0 && <Empty>Aún no hay servicios registrados.</Empty>}

        {servicios.length > 0 && (
          <div className="border-t pt-3 flex items-center justify-between" style={bLine}>
            <p className="text-sm font-bold" style={cChar}>Total presupuestado</p>
            <p className="text-lg font-bold" style={cOrange}>{money(total)}</p>
          </div>
        )}
      </div>

      {modal && (
        <Modal title={modal.srv ? "Editar servicio" : "Nuevo servicio externo"} onClose={() => setModal(null)} wide>
          <FormServicio data={data} initial={modal.srv} onSave={guardar} onClose={() => setModal(null)} />
        </Modal>
      )}
    </div>
  );
          ...
          <div className="space-y-2">
            {data.planes.map((p) => (
              <TarjetaPlan key={p.id} plan={p} sedes={data.sedes}
                ...
                onDelete={() => persist({ ...data, planes: data.planes.filter((x) => x.id !== p.id) })} />
            ))}
...
function VistaBodega({ data, persist, sedes, editable }) {
  ...
  const setItem = (id, patch) => persist({ ...data, stock: data.stock.map((x) => (x.id === id ? { ...x, ...patch } : x)) });
              ...
              return (
                <tr key={x.id} style={{ background: i % 2 ? COLORS.paper : "white", borderTop: `1px solid ${COLORS.line}` }}>
                  <td className={td} style={{ ...cChar, fontWeight: 600 }}>
                    {editable
                      ? <input value={x.nombre} onChange={(e) => setItem(x.id, { nombre: e.target.value })}
                          className={cellInput} style={bLine} />
                      : x.nombre}
                  </td>
                  <td className={td}>
                    {editable
                      ? <input type="number" min="0" value={x.cantidad}
                          onChange={(e) => setItem(x.id, { cantidad: Number(e.target.value) || 0 })}
                  ...
                  <td className={td} style={cSlate}>
                    {editable
                      ? <input value={x.unidad} onChange={(e) => setItem(x.id, { unidad: e.target.value })}
                          className={cellInput} style={{ borderColor: COLORS.line, width: 74 }} />
                      : x.unidad}
                  </td>
                  <td className={td} style={cSlate}>
                    {editable
                      ? <input type="number" min="0" value={x.minimo}
                          onChange={(e) => setItem(x.id, { minimo: Number(e.target.value) || 0 })}
                          className={cellInput} style={{ borderColor: COLORS.line, width: 62 }} />
                      : x.minimo}
                  </td>
                  {editable && (
                    <td className={td}>
                      <input type="number" min="0" step="0.01" value={x.costoUnitario}
                        onChange={(e) => setItem(x.id, { costoUnitario: Number(e.target.value) || 0 })}
                  ...
                  {editable && (
                    <td className={td}>
                      <DeleteBtn onConfirm={() => persist({ ...data, stock: data.stock.filter((y) => y.id !== x.id) })} />
                    </td>
                  )}
                </tr>
              );
        ...
        <Modal title="Nuevo artículo de bodega" onClose={() => setNuevo(null)}>
          <div className="space-y-3">
            ...
            <button disabled={!nuevo.nombre.trim()}
              onClick={() => {
                persist({ ...data, stock: [...(data.stock || []), { ...nuevo, id: uid("stk"), sedeId, nombre: nuevo.nombre.trim() }] });
                setNuevo(null);
              }}
              className="w-full py-2.5 rounded-md font-semibold text-sm text-white disabled:opacity-40" style={{ background: COLORS.orange }}>
              Agregar a bodega
...
function exportarCSV(filas, data) {
  ...
  const filasCsv = filas.map((h) => {
    ...
    const costo = esServ ? (Number(h.presupuesto) || 0) : costoAprobado(h) + costoConsumos(h);
...
function GrupoHistorico({ tipo, items, data, abiertoInicial }) {
  ...
  const costo = items.reduce((a, h) =>
    a + (h.tipo === "servicio" ? (Number(h.presupuesto) || 0) : costoAprobado(h) + costoConsumos(h)), 0);
...
function generarResumenUnificado(data, sedes, mes) {
  ...
  const vinetas = sedes.map((s) => {
    ...
    const serviciosMes = (data.servicios || []).filter((x) => x.sedeId === s.id && mesKey(x.fecha) === mes && Number(x.presupuesto) > 0);
...
const costoActividad = (a) =>
  a.tipo === "servicio" ? (Number(a.presupuesto) || 0) : costoAprobado(a) + costoConsumos(a);
...
function AdminUsuarios({ data, persist }) {
  ...
  const guardar = (u) => {
    ...
    persist({ ...data, usuarios: existe ? data.usuarios.map((x) => (x.id === u.id ? u : x)) : [...data.usuarios, u] });
  };
          ...
          <div key={rolId} className="mb-5">
            ...
            <div className="space-y-2">
              {usuarios.map((u) => <FilaUsuario key={u.id} user={u} data={data} onEdit={() => setModal({ user: u })}
                onDelete={() => persist({ ...data, usuarios: data.usuarios.filter((x) => x.id !== u.id) })} />)}
            </div>
          </div>
        );
      })}
...
function VistaAdmin({ data, persist, user, onLogout, ultimaSync }) {
  ...
  const tabs = [
    ...
    { id: "correctivos", label: "Correctivos", icon: <AlertTriangle size={14} /> },
  ...
  return (
   <ProveedorDetalle data={data}>
    <div className="max-w-6xl mx-auto px-4 pb-16">
      ...
      {tab === "programacion" && <AdminProgramacion data={data} persist={persist} />}
      {tab === "correctivos" && <AdminCorrectivos data={data} persist={persist} />}
      {tab === "servicios" && <AdminServicios data={data} persist={persist} />}
      ...
      {tab === "historico" && <VistaHistorico data={data} sedes={data.sedes} rol="admin" />}
      ...
      {planModal && (
        <Modal title={planModal.plan ? "Editar plan" : "Nuevo plan"} onClose={() => setPlanModal(null)} wide>
          <FormPlan data={data} initial={planModal.plan}
            onAddCategoria={(c) => persist({ ...data, categorias: [...(data.categorias || CATEGORIAS_BASE), c] })}
            onSave={(plan) => {
              const existe = data.planes.some((p) => p.id === plan.id);
              persist({ ...data, planes: existe ? data.planes.map((p) => (p.id === plan.id ? plan : p)) : [...data.planes, plan] });
            }}
